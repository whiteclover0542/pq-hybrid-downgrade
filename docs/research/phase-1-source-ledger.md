...
- `검증 전`: 후보 출처만 등록된 상태
- `검증됨`: 공식 원문에서 주장과 서지·취약점 정보를 직접 확인하고 사람 최종 확인까지 완료한 상태
...
- `LIT-02`: 공식 arXiv 페이지에서 제목·저자(Bhanwar Gupta, Sanjeev Rana)·제출일(2026-09-18)·초록·식별자(arXiv:2609.21273)를 확인했다. 에이전트 자동 열람 + 사람 최종 확인(2026-09-22) 완료로 `검증됨`이다.
- `CVE-01`: OpenSSL 공식 security advisory에서 공개일·영향 버전·`DEFAULT`/tuple/HRR 메커니즘·수정 커밋을 확인했다. 에이전트 자동 열람 + 사람 최종 확인(2026-09-22) 완료로 `검증됨`이다.
- `CVE-02`~`CVE-04`: Red Hat·Cisco·NGINX/F5 공식 원문과 NVD 페이지를 사람이 직접 열어(2026-09-22) CVE 번호·제품·설명 일치를 확인했다. 배경 사례로만 쓰므로 `검증됨(배경 한정)`이다.
- 게이트는 2026-09-22에 통과했다(불일치·접근 불가 항목 없음). 이후 어느 출처든 내용이 바뀌거나 링크가 끊기면 해당 행을 `불일치`/`제외`로 바꾸고 `phase-1-synthesis.md`의 완료 기준과 가설을 다시 판정한다.
